...
class Sidebar_Manager {
	...
	public function add_sidebar( $name, $id = '' ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			return new \WP_Error( 'sidebar-exists', esc_attr__( 'Sidebar with the same ID is already registered.', 'wise-blog' ) );
		}
	...
	public function remove_sidebar( $id ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			...
			return new \WP_Error( 'sidebar-not-found', esc_attr__( 'Sidebar not found.', 'wise-blog' ) );
		}
