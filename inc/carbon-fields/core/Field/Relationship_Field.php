		...
		<div class="relationship-container">
			<div class="selected-items-container">
				<strong>
					...
					<span class="selected-label" data-single-label="<?php esc_attr_e( 'selected item', 'wise-blog' ); ?>" data-plural-label="<?php esc_attr_e( 'selected items', 'wise-blog' ); ?>">
						<?php esc_html_e( 'selected items', 'wise-blog' ); ?>
					</span>
					...
					?>
					<# if ( max !== -1 ) { #>
						<span class="remaining"><?php esc_html_e( 'out of', 'wise-blog' ); ?> {{{ max }}}</span>
					<# } #>
				</strong>
			...
			<div class="relationship-left">
				<div class="search-field">
					<input type="text" class="search-field" placeholder="<?php esc_attr_e( 'Search', 'wise-blog' ); ?>" />
				</div>

				<ul class="relationship-list">
					<# if (options) { #>
						<# _.each(options, function(item) { #>
							<?php echo esc_attr($this->item_template( false )); ?>
						<# }); #>
					<# } #>
				</ul>
			</div>
			...
			<div class="relationship-right">
				<label><?php esc_html_e( 'Associated:', 'wise-blog' ); ?></label>

				<ul class="relationship-list">
					<# if (value) { #>
						<# _.each(value, function(item) { #>
							<?php echo esc_attr($this->item_template()); ?>
						<# }); #>
					<# } #>
				</ul>
			</div>
	...
	public function item_template( $display_input = true ) {
		...
		<li>
			...
			<a href="#" data-item-id="{{{ item.id }}}" data-item-title="{{{ item.title }}}" data-item-type="{{{ item.type }}}" data-item-subtype="{{{ item.subtype }}}" data-item-label="{{{ item.label }}}" data-value="{{{ item.id }}}">
				<# if ( item.edit_link ) { #>
					<em class="edit-link" data-href="{{{ item.edit_link }}}"><?php esc_html_e( 'Edit', 'wise-blog' ); ?></em>
				<# } #>
				<em>{{{ item.label }}}</em>
				<span></span>
				{{{ item.title }}}
