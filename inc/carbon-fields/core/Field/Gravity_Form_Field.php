...
class Gravity_Form_Field extends Select_Field {
	...
	public function setup_gravity_form_options() {
		...
		$options = array(
			'0' => esc_attr__( 'No form', 'wise-blog' ),
		);
	...
	public function template() {
		...
		if ( ! $this->is_plugin_active() ) {
			?><em><?php esc_html_e( 'Please install Gravity Forms plugin', 'wise-blog' ); ?></em><?php
			return;
		}

		// No forms have been found
		if ( empty( $this->options ) ) {
			?><em><?php esc_html_e( 'No Gravity Forms have been found.', 'wise-blog' ); ?></em><?php
			return;
		}
