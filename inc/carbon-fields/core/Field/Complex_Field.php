...
class Complex_Field extends Field {
	...
	public function template_group() {
		...
		<div id="carbon-{{{ complex_name }}}-complex-container" class="carbon-row carbon-group-row" data-group-id="{{ id }}">
			...
			<div class="carbon-group-actions">
				<a class="carbon-btn-collapse" href="#" title="<?php esc_attr_e( 'Collapse/Expand', 'wise-blog' ); ?>">
					<?php esc_html_e( 'Collapse/Expand', 'wise-blog' ); ?>
				</a>

				<a class="carbon-btn-duplicate" href="#" title="<?php esc_attr_e( 'Clone', 'wise-blog' ); ?>">
					<?php esc_html_e( 'Clone', 'wise-blog' ); ?>
				</a>

				<a class="carbon-btn-remove" href="#" title="<?php esc_attr_e( 'Remove', 'wise-blog' ); ?>">
					<?php esc_html_e( 'Remove', 'wise-blog' ); ?>
				</a>
			</div>
	...
	public function set_layout( $layout ) {
		...
		if ( $layout === self::LAYOUT_LIST ) {
			_doing_it_wrong( __METHOD__, sprintf( wp_kses_post( __( 'Complex field <code>%s</code> layout is deprecated&#44; please use <code>set_width()</code> instead.', 'wise-blog') ), self::LAYOUT_LIST  ), null );
		}
