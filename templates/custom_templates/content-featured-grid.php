...
<?php if (!is_paged()): ?>
	<?php
		global $wise_feat_num, $wise_slider_categ, $wise_feat_id, $do_not_duplicate, $wise_slider_orderby;
		...
		if( $wise_feat_id != null ) {
			$feat_grid1_query = new WP_Query( array ( 'post_type' => 'post', 'post__in' => $feat_id, 'category_name' => $wise_slider_categ, 'offset' => 0, 'orderby' => $wise_sliderorder_in, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'post__not_in' => $do_not_duplicate, 'order' => $ascdesc ) );
		} else {
			$feat_grid1_query = new WP_Query( array ( 'post_type' => 'post', 'meta_query' => wise_featured_post(), 'category_name' => $wise_slider_categ, 'offset' => 0, 'orderby' => $wise_sliderorder, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'post__not_in' => $do_not_duplicate, 'order' => $ascdesc ) );
		}
		?>
	<?php if( $wise_feat_num > 0 ) : ?>
	<?php while( $feat_grid1_query -> have_posts() ) : $feat_grid1_query -> the_post(); $do_not_duplicate[] = get_the_ID(); ?>
	...
	<?php
		...
		if( $wise_feat_id != null ) {
			$feat_grid2_query = new WP_Query( array ( 'post__in' => $feat_id_sub, 'category_name' => $wise_slider_categ, 'offset' => 0, 'orderby' => $wise_sliderorder_in, 'posts_per_page' => $number_featured, 'ignore_sticky_posts' => 1, 'post__not_in' => $do_not_duplicate, 'order' => $ascdesc ) );
		} else {
			$feat_grid2_query = new WP_Query( array ( 'post__in' => $wise_get_secondID, 'category_name' => $wise_slider_categ, 'offset' => 0, 'orderby' => $wise_sliderorder, 'posts_per_page' => $number_featured, 'ignore_sticky_posts' => 1, 'post__not_in' => $do_not_duplicate, 'order' => $ascdesc ) );
		} ?>
	<div class="index-wrapper-outer">
		<div class="index-wrapper-grid">
			<?php while( $feat_grid2_query -> have_posts() ) : $feat_grid2_query -> the_post(); $do_not_duplicate[] = get_the_ID(); ?>
